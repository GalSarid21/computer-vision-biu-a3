...
class GaussianDiffusion(nn.Module):
    ...
    def predict_start_from_noise(self, x_t, t, noise):
        ...
        # rearranging x_t as: sqrt_alpha * x_0 + sqrt_one_minus * noise
    ...
    def p_sample(self, x_t, t: int, model_kwargs={}):
        ...
        # get the correct x_start by the objective
        if self.objective == "pred_noise":
            x_start = self.predict_start_from_noise(x_t, t, model_output)
        else:
            x_start = model_output

        # clamp predicted x_start to [-1, 1] for stability
        x_start.clamp_(-1., 1.)

        # get posterior mean and std using x_start
        model_mean, model_std = self.q_posterior(x_start, x_t, t)

        # sample from the posterior
        noise = torch.randn_like(x_t)
        
        # checking the first element of t since all elements in the batch are
        # the same scalar t
        if t[0] > 0:
    ...
    def q_sample(self, x_start, t, noise):
        ...
        # extract coefficient for the current batch of timesteps 't'
        sqrt_alphas_cumprod_t = extract(self.sqrt_alphas_cumprod, t, x_start.shape)
        sqrt_one_minus_alphas_cumprod_t = extract(self.sqrt_one_minus_alphas_cumprod, t, x_start.shape)
        # apply the reparameterization trick: mean + std * noise
    ...
    def p_losses(self, x_start, model_kwargs={}):
        ...
        # get model output for x_t
        model_output = self.model(x_t, t, model_kwargs)

        # compute weighted MSE loss
        loss = ((model_output - target) ** 2) * loss_weight
        loss = loss.mean()
