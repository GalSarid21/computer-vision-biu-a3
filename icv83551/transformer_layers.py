...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        # compute division term in log space for numerical stability:
        # exp(-2i * log(10000) / d_model)
        div_term = torch.exp(torch.arange(0, embed_dim, 2).float() * (-math.log(10000.0) / embed_dim))
        
        # apply sin / cos for even / odd components
    ...
    def forward(self, x):
        ...
        """
        N, S, D = x.shape
        # slice the pe vector to the relevant input sequence length
        output = x + self.pe[:, :S, :]
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        # split values for different heads
        ...
        if attn_mask is not None:
            # apply attention mask if its not None
        ...
        # concatenate heads for final projections
        output_heads = output_heads.transpose(1, 2).contiguous().view(N, S, E)
        # final linear projection
        output = self.proj(output_heads)
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self, tgt, memory, tgt_mask=None):
        ...
        #   - query comes from the decoder (tgt)
        #   - key and value come from the encoder (memory).
        shortcut = tgt
...
class PatchEmbedding(nn.Module):
    ...
    def forward(self, x):
        ...
        # reshape and separate patch dimensions from spatial grid dimensions
        x = x.reshape(N, C, num_patches_h, P, num_patches_w, P)

        # permute to bring the grid dimensions (num_patches) together and 
        # the patch pixel dimensions together
        x = x.permute(0, 2, 4, 1, 3, 5)

        # flatten the grid into a sequence and the pixels into a vector
        x = x.reshape(N, num_patches_h * num_patches_w, C * P * P)
