...
def get_similarity_no_loop(text_features, image_features):
    ...
    text_norm = nn.functional.normalize(text_features, p=2, dim=1)
    image_norm = nn.functional.normalize(image_features, p=2, dim=1)
    # compute dot product
...
def clip_zero_shot_classifier(clip_model, clip_preprocess, images,
    ...
    # tokenize the provided class text descriptions
    text_tokens = clip.tokenize(class_texts).to(device)

    # convert input numpy arrays to PIL Images, apply preprocessing, and stack
    preprocessed_images = [clip_preprocess(Image.fromarray(img)) for img in images]
    image_input = torch.stack(preprocessed_images).to(device)
    ...
    # calculate dot product (cosine similarity) between every image and every text class
    similarity = image_features @ text_features.T

    # get preds: select the index of the highest similarity score for each image
    best_class_indices = similarity.argmax(dim=-1)

    # map the indices back to the original class text strings
    pred_classes = [class_texts[idx] for idx in best_class_indices.cpu().numpy()]
...
class CLIPImageRetriever:
    ...
    def __init__(self, clip_model, clip_preprocess, images, device):
        ...
        # store the features for later retrieval
    ...
    def retrieve(self, query: str, k: int = 2):
        ...
        # fetch top k: we take element zero ([0)]) since similarity has shape (1, N)
        values, indices = similarity[0].topk(k)

        # convert tensor indices to a list of integers
...
class DINOSegmentation:
    def __init__(self, device, num_classes: int, inp_dim : int = 384):
        ...
        """
        # set model as a single layer linear classifier
        hidden_dim = 128
        ...
        # set criterion (loss) as CrossEntropyLoss
    ...
    def train(self, X_train, Y_train, num_iters=500):
        ...
        # setups
        self.model.train()
        inputs = X_train.to(self.device).float()
        targets = Y_train.to(self.device).long()
